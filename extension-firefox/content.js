/**
 * Enhanced content script v4.0 for Firefox with real-time WebSocket updates
...
class InstagramReelsExtensionFirefox {
  constructor() {
...
let extensionInstanceFirefox = null;

function initializeExtensionFirefox() {
  if (extensionInstanceFirefox) {
    extensionInstanceFirefox.stopObserving();
  }
  extensionInstanceFirefox = new InstagramReelsExtensionFirefox();
}

// Initialize when DOM is ready
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", initializeExtensionFirefox);
} else {
  initializeExtensionFirefox();
}

// Also initialize on window load for safety
window.addEventListener("load", initializeExtensionFirefox);

// Handle page visibility changes for real-time updates
document.addEventListener("visibilitychange", () => {
  if (!document.hidden && extensionInstanceFirefox?.isInitialized) {
    // Refresh connection status when page becomes visible
    extensionInstanceFirefox.queuePanel.updateRealTimeIndicator(true);
  }
});

// Export for debugging
if (typeof window !== "undefined") {
  window.extensionInstanceFirefox = extensionInstanceFirefox;
}
